 ...
 *
 */
public class XLSReaderImpl2 implements XLSReader {
	...
	public List<StockData> ReadExcel(String name) {
		...
		try {
			...
			for(int rowNum = 1; rowNum<lastRow; rowNum++){
				...
				data.setDate(row.getCell(0).getStringCellValue().replaceAll("\"", "").split(",")[1]);//时间
				data.setOpeningPrice(new BigDecimal(row.getCell(0).getStringCellValue().replaceAll("\"", "").split(",")[2]));//开盘价
				data.setClosingPrice(new BigDecimal(row.getCell(0).getStringCellValue().replaceAll("\"", "").split(",")[5]));//收盘价
				dataList.add(data);
			}
		} catch (IOException e) {
	...
	public Item analysisDataList(List<StockData> dataList, Item i, int paramDate) {
		...
		startDate = startDate.replaceFirst("-\\d{2}$", "-01");
...
//				System.out.print("买入日期为 " + currentDate +"  \n");
